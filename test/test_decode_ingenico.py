...
class OctetStringDecoderTestCase(unittest.TestCase):
    def testT4F(self):
        self.assertEquals(ing_dec('T4F:07:hA0000000031010\x1c'), 
                          (ApplicationDedicatedFileName(value='\xa0\x00\x00\x00\x03\x10\x10'), ''))
    def testT50(self):
    ...
    def testT57(self):
        self.assertEquals(ing_dec('T57:13:h4888930000000000D12341234432143214321F\x1c'),
                          (Track2EquivalentData(value='H\x88\x93\x00\x00\x00\x00\x00\xd1#A#D2\x142\x142\x1f'), ''))
...
class Decode3303TestCase(unittest.TestCase):
    ...
    def testT4F(self):
        self.assertEqual(self.decodelist[0], '\xa0\x00\x00\x00\x03\x10\x10')

    def testT50(self):
        self.assertEqual(self.decodelist[1], 'Visa Credit')

    def testT57(self):
        self.assertEqual(self.decodelist[2], '\x48\x88\x93\x00\x00\x00\x00\x00\xd0\x00\x00\x00\x00\x00\x00\x00\x00\x00\x0f')

    def testT82(self):
        self.assertEqual(self.decodelist[3], '\x1c\x00')

    def testT84(self):
        self.assertEqual(self.decodelist[4], '\xa0\x00\x00\x00\x03\x10\x10')

    def testT95(self):
        self.assertEqual(self.decodelist[5], '\x80\x00\x00\x00\x00')

    def testT9A(self):
        self.assertEqual(self.decodelist[6], '\x16\x01\x14')

    def testT9B(self):
        self.assertEqual(self.decodelist[7], '\x68\x00')

    def testT9C(self):
        self.assertEqual(self.decodelist[8], '\x00')

    def testT5F2A(self):
        self.assertEqual(self.decodelist[9], '\x08\x40')

    def testT5F34(self):
        self.assertEqual(self.decodelist[10], '\x00')

    ##  one two skip a few

    def testT9F53(self):
        self.assertEqual(self.decodelist[30], '\x52')
...
class Decode3305TestCase(unittest.TestCase):
    ...
    def testT9F53(self):
        self.assertEqual(self.decodelist[38], '\xdc\x40\x04\xf8\x00')
